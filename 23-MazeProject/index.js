//REFACTORED FOR RECTANGULAR MAZES
//SIMPLIFIED VERSION (FOR SQUARE MAZES) IS ON index1.js
...
//square maze is far more easier to implement than rectangular ones
// const width = 600;
// const height = 600;
const width = window.innerWidth - 5; //-5 used for not showing scrolls
const height = window.innerHeight - 5; //-5 used for not showing scrolls
//const cells = 5; //number of cells in a row or column
const cellsHorizontal = 20; //number of cells in a row (number of columns in the maze)
const cellsVertical = 10; //number of cells in a column (number of rows in the maze)

const borderWallWidth = 2; //outer walls
const innerWallWidth = 5;

//const unitLength = width / cells; //unitWidth & unitHeight of each cell
const unitLengthX = width / cellsHorizontal; //unitWidth of each cell
const unitLengthY = height / cellsVertical; //unitHeight of each cell
...
const startRow = Math.floor(Math.random() * cellsVertical); //cellsVertical: number of cells in a column (number of rows in the maze)
const startCol = Math.floor(Math.random() * cellsHorizontal); //cellsHorizontal: number of cells in a row (number of columns in the maze)
